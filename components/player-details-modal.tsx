    ...
    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-md p-0 overflow-hidden border-border/20 bg-background/95 backdrop-blur-3xl outline-none rounded-[2rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.3)]">
                {/* Minimal Header */}
                <div className="relative border-b border-border/10 px-8 py-8">
                    <div className="flex items-center justify-between">
                        <div className="space-y-1">
                            <h2 className="text-3xl font-black tracking-tight text-foreground uppercase italic">{player.name}</h2>
                            <div className="flex items-center gap-2">
                                <span className="h-1.5 w-8 rounded-full bg-primary" />
                                <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-primary">Technical Profile</p>
                            </div>
                        </div>
                        <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-secondary/50 border border-border/10 text-muted-foreground/60 backdrop-blur-md">
                ...
                <div className="p-8 space-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="relative overflow-hidden rounded-2xl border border-border/10 bg-secondary/20 p-6 transition-all duration-300 hover:bg-secondary/30">
                            ...
                            <span className="text-4xl font-black text-foreground tabular-nums tracking-tighter">{player.goals}</span>
                        </div>
                        <div className="relative overflow-hidden rounded-2xl border border-border/10 bg-secondary/20 p-6 transition-all duration-300 hover:bg-secondary/30">
                            <div className="absolute -right-2 -top-2 opacity-[0.05]">
                            ...
                            <span className="text-4xl font-black text-foreground tabular-nums tracking-tighter">{player.matches}</span>
                        </div>
                        <div className="col-span-2 relative overflow-hidden rounded-2xl border border-border/10 bg-primary/5 p-6 border-l-4 border-l-primary">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <span className="text-3xl font-black text-primary tabular-nums tracking-tighter">{player.goalsPerMatch.toFixed(2)}</span>
                                </div>
